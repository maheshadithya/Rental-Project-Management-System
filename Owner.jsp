...
%>
				<option value="<%out.println(resultSet.getString("owner_id"));%>"><%out.println(resultSet.getString("owner_id"));%></option>									
<%
}
conn.close();
} 
...
%>		
				...
				function SelectFunction() {
					...
					window.location.replace("Owner.jsp?owner_id="+id_selected)
				}
				</script>
...
String sql1 ="SELECT * from owner WHERE owner_id='"+owner_id+"'";
resultSet1 = stmt1.executeQuery(sql1);
if(resultSet1.next()){
%>			
			
			
			<input type="hidden" value="<%out.println(resultSet1.getString(1));%>" name="owner_id1">
			<br>
			<br>
...
%>
	...
	<table class="center">
			<tr>
				<td><input type="submit" value="Save" name="submit" id="submit" formaction="OwnerSaveProcess.jsp"></td><td>&nbsp;</td>
					<td><input type="submit" value="Delete" name="submit" id="submit" formaction="OwnerDeleteProcess.jsp"></td><td>&nbsp;</td>
				
	</form>
				<form>
					<td><input type="submit" value="New" name="submit" id="submit" formaction="Owner.jsp"></td><td>&nbsp;</td>
					<td><input type="submit" value="Cancel" name="submit" id="submit" formaction="Owner.jsp"></td><td>&nbsp;</td>
...
<%
			if(session.getAttribute("RecordInsertSuccess3")!=null){
				%>
					<script>
						alert("Record Insert Sucess!");  
					</script>
				<%
				session.removeAttribute("RecordInsertSuccess3");
			}
			if(session.getAttribute("RecordDeleteSuccess3")!=null){
				%>
					<script>
						alert("Record Delete Sucess!");  
					</script>
				<%
				session.removeAttribute("RecordDeleteSuccess3");
			}
